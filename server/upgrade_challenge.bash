...
docker run --rm  -i  \
  ...
  neilpang/acme.sh --issue --debug 2 --log --dns dns_ali -d "$DOMAIN" -m acme-$(openssl rand -hex 8)@do8.cc --domain-alias acme-$DOMAIN@do8.cc

yes | cp out/$DOMAIN/$DOMAIN.key ./ssl/$DOMAIN.key
yes | cp out/$DOMAIN/fullchain.cer ./ssl/$DOMAIN.fullchain.cer
